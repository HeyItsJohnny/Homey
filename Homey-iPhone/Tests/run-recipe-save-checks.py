...
source = (root / 'Homey-iPhone/Features/Meals/RecipeImportDiagnostics.swift').read_text() + '\n' + (root / 'Homey-iPhone/Features/Meals/RecipeImageReference.swift').read_text() + '\n' + models + '\n' + (root / 'Homey-iPhone/Features/Meals/RecipeImportSupport.swift').read_text() + '\n' + service[service.index('struct SaveMealParams:'):]
...
with tempfile.TemporaryDirectory(prefix='homey-save-contract-') as directory:
    ...
    subprocess.run([str(binary), str(root / 'Tests/Fixtures/front-range-fed-import.json')], check=True)
